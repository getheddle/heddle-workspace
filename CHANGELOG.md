...
- **Project state stays agent-neutral.** New rule — *Agents don't own the work*: project discipline and facts live in shared, committed places (`AGENTS.md`, `roadmap/`, `session-starters/`, repo docs, CI); an agent's private memory, hooks, settings, and permissions hold only agent-specific state; a check the project relies on lives in CI or a repo script, and a hook may only call it; per-agent instruction files are pointers/symlinks, not copies. Added to `templates/workspace-init/AGENTS.md`, `docs/AGENT_ADAPTERS.md` (*What an adapter may hold*), this repo's `AGENTS.md`, and `hooks/README.md` (*Rules for hooks*: no absolute clone paths).
  - **Migration:** existing workspaces keep the root `AGENTS.md` they were generated with. Add the *Agents don't own the work* section from `templates/workspace-init/AGENTS.md` by hand; replace any divergent real `GEMINI.md`/`QWEN.md` with a symlink to `AGENTS.md` (the installer skips existing real files).
- **`/heddle-preflight` now runs checks it was missing.** `heddle`: the wire-contract checks CI runs — `tools/export_schemas.py --check` and `tools/check_envelope_convention.py` (previously preflight passed while CI would fail on schema or envelope-convention drift). `heddle-sdk`: the JVM test (`./jvm/gradlew -p jvm test`, Java 17+), absent since the JVM SDK landed.

### Fixed

- `maintenance-implementer` no longer points at a non-existent `sprint-feedback` skill; it now names the C-M contract skeleton plus the workspace's *Feedback files* convention.
